let counterparties_search = "chainalysis1:search:da77e740-adcf-11ed-bcbb-6bce064db907"
let query = {};
let eids = [];
let entities = [];
...
async function invokedItems(graphSelection) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        let url = '../../api/web_services/chainalysis-iapi/cluster_counterparties/_invokeAndReturn?returnData=true';
        Http.open("POST", url);
...
async function beforeAll(graphId, graphModel, graphSelection) {
    ...
    return await Promise.resolve(invokedItems(graphSelection))
        .then(async function () {
            ...
            let searchResults = await f.executeEsSearch("web-service-chainalysis-iapi-cluster_counterparties-results-cluster", "", query, 500)
            await Promise.resolve(searchResults)
                .then(async function (searchResults) {
                    ...
                    searchResults.hits.hits.map(hit => {
                        entities.push(`web-service-chainalysis-iapi-cluster_counterparties-results-cluster/_doc/${hit._id}`)
                    })
                    console.log(entities)
                    await Promise.resolve(f._fetchVerticesAdd(graphId, entities, counterparties_search))
                        .then(async results => {
                            console.log(results)
