    ...
    async function graphDo() {
        if (invocated == false) {
            ...
            await Promise.all(selectedNodes.map(async node => {
                ...
                const raw_hits = node.payload.siren.nlp.matches.crypto
                var ready_hits = []
                console.log(raw_hits)
                if (typeof raw_hits == 'array') {
                    ready_hits.push.apply(ready_hits, raw_hits)
                }
                else {
                    ready_hits.push.apply(ready_hits, [raw_hits])
                }
                ready_hits.map(async (hit) => {
                    config.assets.map(async (asset) => {
                        console.log('running ' + asset + ' ' + hit)
                        ...
                        console.log(asset + ':' + hit + ' ' + invocation.statusText + ' next: ' + invocation.data.pagination.nextPage)
                        ...
                        if (invocation.data.pagination[0].nextPage) {
                            ...
                            do {
                                ...
                                console.log(asset + ':' + hit + ' pagination ' + more_invocation.statusText + ' next: ' + more_invocation.data.pagination[0].nextPage)
