...
const config = {
    ...
    destination: "Chainalysis Cluster Observations",
    ...
    async function graphDo() {
        ...
        try {
            ...
            await Promise.all(collectedAddresses.map(async address => {
                ...
                if (invocation.data.observation.length > 0) {
                    setResults(results => [...results, ...invocation.data.observation]) // custom for this endpoint
